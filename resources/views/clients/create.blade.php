@extends('layouts.app')
@section('title', __('app.add_new_client'))
...
<section class="content-header">
  <h1>{{ __('app.add_new_client') }}</h1>
  <ol class="breadcrumb">
    <li><a href="{{ route('dashboard') }}">{{ __('app.dashboard') }}</a></li>
    <li><a href="{{ route('users.index') }}">{{ __('app.clients') }}</a></li>
    <li class="active">{{ __('app.add_new_client') }}</li>
  </ol>
</section>
<!-- Main content -->
    ...
    <form class="form-horizontal" id="frm-user">
      ...
      <div class="box-body">
        ...
        <div class="form-group">
          ...
          <div class="col-md-6">
            <input type="password" class="form-control" name="password" placeholder="{{ __('app.confirm_password') }}">
          </div>
          <span></span>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <div class="col-md-offset-2 col-md-10">
            <button type="button" class="btn btn-primary loader" onclick="submitData(this, this.form, 'POST', '{{ route('client.store') }}')" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> {{ __('app.save') }}">{{ __('app.save') }}</button>
            <button type="button" class="btn btn-primary" onclick="submitData(this, this.form, 'POST', '{{ route('client.store') }}', 1)" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> {{ __('app.save_add_new') }}">{{ __('app.save_add_new') }}</button>
