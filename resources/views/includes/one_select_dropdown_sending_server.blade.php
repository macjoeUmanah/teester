...
<select name="sending_server_id" id="sending-servers" class="form-control" >
      ...
      <optgroup label="{{ Helper::decodeString($group->name) }}">
        @foreach($group->sendingServers as $sending_server)
          @if(in_array($sending_server->id, $client_sending_servers))
           <option value="{{ $sending_server->id }}" {{ !empty($sending_server_id) && $sending_server_id == $sending_server->id ? 'selected="selected"' : '' }}>{{ Helper::decodeString($sending_server->name) }}</option>
          @endif
        @endforeach
      </optgroup>
    @endforeach
