...
<div class="modal-content">
  <form class="form-horizontal" id="frm-group">
    <div class="modal-header">
      ...
      <h4 class="modal-title">{{ __('app.edit_custom_field') }}</h4>
    </div>
    <div class="modal-body">
      <div class="box-body margin-bottom-70">
        <div class="form-group">
          ...
          <div class="col-md-9">
            <div class="input-group from-group">
              <input type="text" class="form-control" name="name" value="{{ Helper::decodeString($custom_field->name) }}" placeholder="{{ __('app.custom_field_name') }}">
          ...
          <div class="col-md-9">
            <div class="input-group from-group">
              <select name="type" id="type" class="form-control">
                ...
                <option value="{{ $type }}" {{ $custom_field->type == $type ? 'selected' : '' }}>{{ $name }}</option>
                @endforeach
              </select>
              <div class="input-group-addon input-group-addon-right">
        ...
        <div class="form-group {{$custom_field->type == 'radio' || $custom_field->type == 'checkbox' || $custom_field->type == 'dropdown' ? '' : 'display-none'}}" id='values'>
          <label class="col-md-2 control-label">{{ __('app.custom_field_values') }}</label>
          <div class="col-md-9">
            <textarea class="form-control" name="values" rows="5" placeholder="{{ __('app.custom_field_values_placehoder') }}">{{ $custom_field->values ?? '' }}</textarea>
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="col-md-9">
            <div class="input-group from-group">
              @php $list_ids = $custom_field_lists @endphp
    ...
    <div class="modal-footer">
      <div class="form-group">
        <div class="col-md-offset-2 col-md-10">
          <button type="button" class="btn btn-primary" onclick="submitData(this, this.form, 'PUT', '{{ route('custom_field.update', ['id' => $custom_field->id]) }}');" data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> {{ __('app.save') }}">{{ __('app.save') }}</button>
